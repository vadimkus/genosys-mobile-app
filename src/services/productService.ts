...
export class ProductService {
      ...
      if (
        ...
        if (this.products.length > 0) {
            ...
            this.newProducts = this.products
              .filter(
                product =>
                  product.name
                    .toLowerCase()
                    .includes('bio-ferment age defying powder mask') ||
                  product.name
                    .toLowerCase()
                    .includes('skin reboot pdrn mask pack')
              )
              .map(product => ({
                ...product,
                isNew: true, // Ensure new products are marked as new
              }));
            console.log(
              '⚠️ New products API not available, using specific new arrival products.'
            );
          } else {
          ...
          if (categories.length === 0) {
            console.log(
              '🏷️ Using fallback categories because API categories is empty'
            );
            ...
            console.log(
              '🏷️ Category counts after recalculation:',
            ...
            const sunCategoryExists = this.categories.some(
              c => c.name === 'Sun'
            );
        ...
        console.log(
          `✅ Loaded ${this.products.length} products, ${this.featuredProducts.length} featured, ${this.newProducts.length} new from API`
        );
        ...
        console.log(
          '☀️ Sun products found:',
          sunProducts.map(p => p.name)
        );

        // Debug: Check if Sun category exists
        const sunCategory = this.categories.find(c => c.name === 'Sun');
        console.log(
          '☀️ Sun category exists:',
          !!sunCategory,
          sunCategory ? `count: ${sunCategory.count}` : 'not found'
        );

        // Debug: Check Cushion BB products
        const cushionBBProducts = this.products.filter(
          p => p.category === 'Cushion BB'
        );
        console.log(
          '💄 Cushion BB products found:',
          cushionBBProducts.map(p => p.name)
        );
        console.log(
          '💄 All product categories:',
          this.products.map(p => `${p.name}: ${p.category}`)
        );

        // Debug: Check if Cushion BB category exists
        const cushionBBCategory = this.categories.find(
          c => c.name === 'Cushion BB'
        );
        console.log(
          '💄 Cushion BB category exists:',
          !!cushionBBCategory,
          cushionBBCategory ? `count: ${cushionBBCategory.count}` : 'not found'
        );
        ...
        console.log(
          '🏷️ Fallback categories loaded:',
          this.categories.map(c => `${c.name} (${c.count})`)
        );
        const sunProducts = this.products.filter(p => p.category === 'Sun');
        console.log(
          '☀️ Fallback sun products found:',
          sunProducts.map(p => p.name)
        );
        ...
        const sunCategory = this.categories.find(c => c.name === 'Sun');
        console.log(
          '☀️ Fallback Sun category exists:',
          !!sunCategory,
          sunCategory ? `count: ${sunCategory.count}` : 'not found'
        );
      }
  ...
  private async addEyeCellProducts(): Promise<void> {
    ...
    console.log(
      `📊 Current Eye Care products: ${this.products.filter(p => p.category === 'Eye Care').length}`
    );
    ...
    const eyeCellProducts = [
      {
        ...
        description:
          'Daily eye cream that reduces fine lines, dark circles, and puffiness. Advanced peptide technology.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FEC.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FEC.jpg&w=1200&q=75',
        ],
        category: 'Eye Care',
      ...
      },
      {
        ...
        description:
          'Advanced eye contour serum for anti-aging and dark circle reduction. Professional eye area treatment.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FEYS.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FEYS.jpg&w=1200&q=75',
        ],
        category: 'Eye Care',
      ...
      },
      {
        ...
        description:
          'Professional eye peptide gel patches for intensive eye area treatment. Advanced peptide therapy.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2Feyecell-patch.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2Feyecell-patch.jpg&w=1200&q=75',
        ],
        category: 'Eye Care',
      ...
      },
      {
        ...
        description:
          'Comprehensive eye care kit with multiple components for complete eye area treatment and anti-aging.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2Feyecell-kit.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2Feyecell-kit.jpg&w=1200&q=75',
        ],
        category: 'Eye Care',
      ...
      },
      {
        ...
        description:
          'Professional-grade treatment mask infused with PDRN (Polydeoxyribonucleotide) extracted from salmon DNA. This advanced mask promotes cellular regeneration, accelerates skin repair, and enhances overall skin health.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FPDRN.png&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FPDRN.png&w=1200&q=75',
        ],
        category: 'Mask',
    ...
    eyeCellProducts.forEach(eyeCellProduct => {
      ...
      console.log(
        `🔍 Checking ${eyeCellProduct.name} (ID: ${eyeCellProduct.id}) - Exists: ${exists}`
      );
      if (!exists) {
    ...
    console.log(
      `✅ Added ${addedCount} EyeCell products to existing data (${eyeCellProducts.length - addedCount} already existed)`
    );
    console.log(`📊 Final products count: ${this.products.length}`);
    console.log(
      `📊 Final Eye Care products: ${this.products.filter(p => p.category === 'Eye Care').length}`
    );
  }
    ...
    this.products = [
      {
        ...
        description:
          'Premium anti-aging cream with advanced peptides and Korean skincare technology',
        price: 89.99,
        originalPrice: 129.99,
        discountPercentage: 31,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75',
        ],
        category: 'Skincare',
      ...
      },
      {
        ...
        description:
          'Revolutionary hair growth serum with natural ingredients and advanced technology',
        price: 149.99,
      ...
      },
      {
        ...
        description:
          'Advanced multi-functional anti-wrinkle cream with comprehensive skincare benefits',
        price: 129.99,
        originalPrice: 159.99,
        discountPercentage: 19,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FANT.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FANT.jpg&w=1200&q=75',
        ],
        category: 'Cream',
      ...
      },
      {
        ...
        description:
          'Powerful multi-functional anti-wrinkle serum for advanced skincare treatment',
        price: 149.99,
        originalPrice: 179.99,
        discountPercentage: 17,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FMSSS.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FMSSS.jpg&w=1200&q=75',
        ],
        category: 'Serum',
      ...
      },
      {
        ...
        description:
          'Comprehensive sun protection cream with SPF 40 and PA++ rating for complete UV protection',
        price: 89.99,
        originalPrice: 119.99,
        discountPercentage: 25,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FSSUN.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FSSUN.jpg&w=1200&q=75',
        ],
        category: 'Sun',
      ...
      },
      {
        ...
        description:
          'Multi-vitamin radiance cream for bright, glowing skin with essential vitamins',
        price: 109.99,
        originalPrice: 139.99,
        discountPercentage: 21,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FRAA.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FRAA.jpg&w=1200&q=75',
        ],
        category: 'Cream',
      ...
      },
      {
        ...
        description:
          'Professional microneedling device for advanced skin rejuvenation and collagen stimulation',
        price: 199.99,
        originalPrice: 249.99,
        discountPercentage: 20,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2Fgenosys-microneedling-devices.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2Fgenosys-microneedling-devices.jpg&w=1200&q=75',
        ],
      ...
      },
      {
        ...
        name: 'ND Cell ANTI-WRINKLE CREAM',
        description:
          'Advanced anti-wrinkle cream with ND Cell technology for deep skin rejuvenation and wrinkle reduction',
        price: 89.99,
        originalPrice: 119.99,
        discountPercentage: 25,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FND.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FND.jpg&w=1200&q=75',
        ],
        category: 'Cream',
      ...
      },
      {
        ...
        description:
          'Advanced skin barrier protecting cream with ceramides and hyaluronic acid for deep hydration and protection',
        price: 89.99,
        originalPrice: 119.99,
        discountPercentage: 25,
      ...
      },
      {
        ...
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FNeedle-pen.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FNeedle-pen.jpg&w=1200&q=75',
        ],
        category: 'Device',
      ...
      },
      {
        ...
        description:
          'Professional anti-aging ampoule for microneedling treatments. Advanced peptide formula to reduce wrinkles and improve skin firmness for youthful appearance.',
        price: 580.0,
        originalPrice: 580.0,
        discountPercentage: 0,
        imageUrl:
      ...
      },
      {
        ...
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FGENTRON.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FGENTRON.jpg&w=1200&q=75',
        ],
        category: 'Device',
      ...
      },
      {
        ...
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHAIRGENBOOSTER.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHAIRGENBOOSTER.jpg&w=1200&q=75',
        ],
        category: 'Device',
      ...
      },
      {
        ...
        name: 'Geno LED IR II',
        description:
          'Professional LED light therapy device with infrared technology for advanced skin treatment',
        price: 399.99,
        originalPrice: 499.99,
        discountPercentage: 20,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FLEDD.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FLEDD.jpg&w=1200&q=75',
        ],
        category: 'Device',
      ...
      },
      {
        ...
        description:
          'Intensive moisture replenishing cream with hyaluronic acid for deep hydration and skin nourishment',
        price: 89.99,
        originalPrice: 119.99,
        discountPercentage: 25,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHER.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHER.jpg&w=1200&q=75',
        ],
        category: 'Cream',
      ...
      },
      {
        ...
        description:
          'Premium hair and scalp solution with advanced peptide technology. Prevents hair loss, promotes regrowth, and strengthens hair follicles with botanical extracts.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3.jpg&w=1200&q=75',
        ],
        category: 'Scalp/Hair',
      ...
      },
      {
        ...
        description:
          'Complete hair and scalp treatment system with scalp peeling, hair solution, and roller device. Prevents hair loss, stimulates regrowth, and promotes scalp health.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3KIT.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3KIT.jpg&w=1200&q=75',
        ],
        category: 'Scalp/Hair',
      ...
      },
      {
        ...
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHYDRO.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHYDRO.jpg&w=1200&q=75',
        ],
        category: 'Mask',
      ...
      },
      {
        ...
        description:
          'Specialized hair tonic with peptide technology and botanical extracts. Strengthens hair follicles, improves scalp health, and promotes hair growth.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3TONIC.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3TONIC.jpg&w=1200&q=75',
        ],
        category: 'Scalp/Hair',
      ...
      },
      {
        ...
        description:
          'Gentle scalp peeling solution for microneedling treatment preparation. Removes keratinized particles, provides refreshing cooling sensation, and optimizes scalp condition for enhanced treatment absorption.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3PEELING.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3PEELING.jpg&w=1200&q=75',
        ],
        category: 'Scalp/Hair',
      ...
      },
      {
        ...
        description:
          'Professional functional shampoo for hair loss prevention and scalp health. KFDA-approved with advanced ingredient complex for optimal hair growth.',
        price: 0, // Login to see price
        originalPrice: 0,
        discountPercentage: 0,
        imageUrl:
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3SHAMPOO.jpg&w=1200&q=75',
        imageUrls: [
          'https://genosys.ae/_next/image?url=%2Fimages%2FHR3SHAMPOO.jpg&w=1200&q=75',
        ],
        category: 'Scalp/Hair',
  ...
  private transformProducts(dbProducts: any[]): Product[] {
    return dbProducts.map(product => {
      ...
      const getCategory = (productName: string) => {
        ...
        if (
          name.includes('sun cream') ||
          name.includes('ultra shield') ||
      ...
      const getBrand = (productName: string) => {
        ...
        // All Genosys products
        if (name.includes('all for sensitive') ||
            name.includes('eyecell') ||
            name.includes('geno-led') ||
            name.includes('hr³ matrix') ||
            name.includes('hair-gentron') ||
            name.includes('hairgen') ||
            name.includes('intensive') ||
            name.includes('microbiome') ||
            name.includes('moisture replenishing') ||
            name.includes('multi functional') ||
            name.includes('multi sun') ||
            name.includes('multi vita') ||
            name.includes('nd cell') ||
            name.includes('peptide gel') ||
            name.includes('power solution') ||
            name.includes('problem control') ||
            name.includes('skin barrier') ||
            name.includes('skin caring') ||
            name.includes('skin defender') ||
            name.includes('skin reboot') ||
            name.includes('skin renewal') ||
            name.includes('skin rescue') ||
            name.includes('snow') ||
            name.includes('soothing') ||
            name.includes('ultra shield') ||
            name.includes('bio-ferment') ||
            name.includes('egf repair') ||
            name.includes('epi turnover') ||
            name.includes('ez co₂') ||
            name.includes('hydro cool') ||
            name.includes('microneedle') ||
            name.includes('needle pen')) {
          return 'Genosys';
        }
        
        // Default brand
        return product.brand || 'Genosys';
      };

      product.brand = getBrand(product.name);

      // Debug: Log category mapping for sun products
      if (
        product.name.toLowerCase().includes('sun') ||
        product.name.toLowerCase().includes('ultra shield')
      ) {
        console.log(
          `☀️ Category mapping for ${product.name}: ${originalCategory} -> ${product.category}`
        );
      }
      ...
      if (
        ...
        product.name.toLowerCase().includes('blemish balm') ||
        product.name.toLowerCase().includes('bb')
      ) {
        console.log(
          `💄 Category mapping for ${product.name}: ${originalCategory} -> ${product.category}`
        );
      }
      ...
      const getImageUrl = (productName: string) => {
        ...
        if (
          ...
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FBFAD.png&w=1200&q=75';
        if (name.includes('skin reboot pdrn mask pack'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPDRN.png&w=1200&q=75';
        if (name.includes('eyecell eye zone care kit'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FEYEZ.jpg&w=1200&q=75';
        if (name.includes('geno-led ir ii'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FLEDD.jpg&w=1200&q=75';
        if (name.includes('hair-gentron'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fgen.jpg&w=1200&q=75';
        ...
        if (
          ...
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHHR.jpg&w=1200&q=75';
        if (
          name.includes('hr³ matrix hair tonic') ||
          name.includes('hair tonic')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHT.jpg&w=1200&q=75';
        if (
          name.includes('hr³ matrix mesopecia kit') ||
          name.includes('mesopecia kit')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fmeso.jpg&w=1200&q=75';
        if (
          name.includes('hr³ matrix scalp peeling') ||
          name.includes('scalp peeling')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fscal.jpg&w=1200&q=75';
        if (
          name.includes('hr³ matrix scalp shampoo') ||
          name.includes('scalp shampoo')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSham.jpg&w=1200&q=75';
        if (name.includes('hairgen booster'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FBooster.jpg&w=1200&q=75';

        // Cushion BB products (must come before general blemish balm)
        if (name.includes('skin caring blemish balm cushion'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FBBC.jpg&w=1200&q=75';

        // Skincare creams
        if (
          name.includes('blemish balm cream') ||
          name.includes('blemish balm cushion')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FBLEM.jpg&w=1200&q=75';
        if (name.includes('multi functional') && name.includes('cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FANT.jpg&w=1200&q=75';
        if (name.includes('anti-wrinkle cream') || name.includes('nd cell'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        if (name.includes('moisture replenishing') && name.includes('cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHER.jpg&w=1200&q=75';
        if (name.includes('intensive problem control cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPRB.jpg&w=1200&q=75';
        if (name.includes('intensive hydro soothing cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHSC.jpg&w=1200&q=75';
        if (name.includes('skin barrier protecting cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        if (name.includes('egf repair oxymask cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FEGF.jpg&w=1200&q=75';
        if (name.includes('soothing repair postcream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        if (name.includes('eyecell eye contour cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FEC.jpg&w=1200&q=75';

        // Sun protection
        if (name.includes('sun cream') || name.includes('ultra shield'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSSUN.jpg&w=1200&q=75';

        // Masks
        if (name.includes('ez co₂ mask kit'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FEZE.jpg&w=1200&q=75';
        if (name.includes('skin rescue overnight cream mask'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSKIN.jpg&w=1200&q=75';
        if (name.includes('peptide gel mask'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPEP.jpg&w=1200&q=75';
        if (name.includes('soothing bomb sea algae mask'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSEA.jpg&w=1200&q=75';
        if (name.includes('hydro cool modeling mask'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHYDR.jpg&w=1200&q=75';

        // Eye care
        if (name.includes('eyecell eye peptide gel patch'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPatch.jpg&w=1200&q=75';

        // Serums
        if (name.includes('multi functional') && name.includes('serum'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FMSSS.jpg&w=1200&q=75';
        if (name.includes('multi vita radiance serum'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FRADS.jpg&w=1200&q=75';
        if (name.includes('problem control serum'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        if (name.includes('all for sensitive serum'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FASE.jpg&w=1200&q=75';
        if (name.includes('moisture replenishing') && name.includes('serum'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHRS.jpg&w=1200&q=75';
        if (name.includes('eyecell eye contour serum'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FEYS.jpg&w=1200&q=75';

        // Boosters and toners
        if (name.includes('snow booster'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        if (name.includes('intensive problem control toner'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPRS.jpg&w=1200&q=75';
        if (name.includes('microbiome energy infusing mist'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fmist.jpg&w=1200&q=75';

        // Peeling and cleansing
        if (name.includes('skin renewal peeling system'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        if (name.includes('epi turnover boosting peeling gel'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FEPI.jpg&w=1200&q=75';
        if (name.includes('skin defender lip & eye makeup remover'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';
        ...
        if (name.includes('power solution cts'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FCTS.jpg&w=1200&q=75';
        if (name.includes('power solution cvs'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FCVS.jpg&w=1200&q=75';
        if (name.includes('power solution hes'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FHES.jpg&w=1200&q=75';
        if (name.includes('power solution'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fin.png&w=1200&q=75';

        // Devices
        if (name.includes('microneedle roller'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2Fgenosys-microneedling-devices.jpg&w=1200&q=75';
        // Multi vita radiance cream
        if (name.includes('multi vita radiance cream'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FRAA.jpg&w=1200&q=75';

        // ND Cell ANTI-WRINKLE CREAM
        if (
          name.includes('nd cell anti-wrinkle cream') ||
          name.includes('ND Cell ANTI-WRINKLE CREAM')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FND.jpg&w=1200&q=75';

        // Needle Pen-K
        if (name.includes('needle pen-k') || name.includes('Needle Pen-K'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FNeedle-pen.jpg&w=1200&q=75';

        // PEPTIDE GEL MASK
        if (
          name.includes('peptide gel mask') ||
          name.includes('PEPTIDE GEL MASK')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPEP.jpg&w=1200&q=75';
        ...
        // POWER SOLUTION AWS
        if (
          name.includes('power solution aws') ||
          name.includes('POWER SOLUTION AWS')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FAWS.jpg&w=1200&q=75';

        // POWER SOLUTION PCS
        if (
          name.includes('power solution pcs') ||
          name.includes('POWER SOLUTION PCS')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPCS.jpg&w=1200&q=75';

        // POWER SOLUTION SWS
        if (
          name.includes('power solution sws') ||
          name.includes('POWER SOLUTION SWS')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSWS.jpg&w=1200&q=75';

        // PROBLEM CONTROL SERUM
        if (
          name.includes('problem control serum') ||
          name.includes('PROBLEM CONTROL SERUM')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FPRSS.jpg&w=1200&q=75';

        // SOOTHING REPAIR POSTCREAM
        if (
          name.includes('soothing repair postcream') ||
          name.includes('SOOTHING REPAIR POSTCREAM')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSRC.jpg&w=1200&q=75';

        // SKIN DEFENDER LIP & EYE MAKEUP REMOVER
        if (
          name.includes('skin defender lip') ||
          name.includes('SKIN DEFENDER LIP')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FDEF.jpg&w=1200&q=75';

        // SNOW O₂ CLEANSER
        if (
          name.includes('snow o₂ cleanser') ||
          name.includes('SNOW O₂ CLEANSER')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSNOW.jpg&w=1200&q=75';

        // SNOW BOOSTER
        if (name.includes('snow booster') || name.includes('SNOW BOOSTER'))
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FBOOS.jpg&w=1200&q=75';

        // SKIN RENEWAL PEELING SYSTEM (SRS)
        if (
          name.includes('skin renewal peeling system') ||
          name.includes('SKIN RENEWAL PEELING SYSTEM')
        )
          return 'https://genosys.ae/_next/image?url=%2Fimages%2FSRS.jpg&w=1200&q=75';
      ...
      if (useLocalMapping) {
        ...
        finalImageUrl =
          'https://genosys.ae/_next/image?url=%2Fimages%2FBRR.jpg&w=1200&q=75';
        ...
        finalImageUrl =
          'https://genosys.ae/_next/image?url=%2Fimages%2FAWS.jpg&w=1200&q=75';
  ...
  private mapProductToCategory(
    ...
    if (
      ...
      name.includes('skin renewal peeling') ||
      name.includes('epi turnover') ||
      name.includes('peeling system')
    ) {
      return 'Peeling';
    }
    ...
    if (
      name.includes('toner') ||
      name.includes('mist') ||
      name.includes('microbiome energy infusing mist') ||
    ...
    if (
      name.includes('serum') ||
      name.includes('all for sensitive') ||
      name.includes('anti-wrinkle serum') ||
      name.includes('problem control serum') ||
    ...
    if (
      name.includes('mask') ||
      name.includes('gel mask') ||
      name.includes('collagen mask') ||
      name.includes('overnight') ||
      name.includes('treatment') ||
      name.includes('ez co₂ mask')
    ) {
      return 'Mask';
    }
    ...
    if (
      ...
      name.includes('anti-wrinkle cream') ||
      name.includes('intensive hydro soothing cream') ||
      name.includes('intensive problem control cream')
    ) {
      return 'Cream';
    }
